  ...
  return `
    <div class="game-row ${isFavorite ? 'favorite-game' : ''}">
      ...
      <div class="game-score-block">
        ${scoreText}
        <span class="game-status ${statusClass}">${game.status === 'Live' ? game.quarter : game.status}</span>
      </div>
...
function renderBacklog() {
  ...
  grid.innerHTML = filtered.map(game => {
    const badgeClass = { playing: 'badge-playing', completed: 'badge-completed', want: 'badge-want' }[game.status] || 'badge-want';
    const badgeLabel = { playing: 'Playing', completed: 'Completed', want: 'Want to play' }[game.status] || game.status;
    return `
...
function addToBacklog(game, status = 'want') {
  if (state.backlog.find(g => g.id === game.id)) return; // already in list
...
if (searchInput) {
  searchInput.addEventListener('input', () => {
    ...
    state.searchTimer = setTimeout(() => searchGames(q), 400);
  });
}
...
async function searchGames(query) {
  ...
  try {
    // RAWG API — free, no auth needed for basic search
    // In production: get a free key at rawg.io and add ?key=YOUR_KEY
    const res = await fetch(`https://api.rawg.io/api/games?search=${encodeURIComponent(query)}&page_size=8`);
    ...
    resultsEl.innerHTML = data.results.map(g => {
      const inList = state.backlog.find(b => b.id === g.id);
      return `
        <div class="search-result-item">
          ${g.background_image ? `<img class="result-cover" src="${g.background_image}" alt="${g.name}" loading="lazy">` : '<div class="result-cover"></div>'}
          <div class="result-info">
            <div class="result-name">${g.name}</div>
            <div class="result-year">${g.released?.split('-')[0] || ''}</div>
          </div>
          <button class="result-add-btn" onclick='handleAddGame(${JSON.stringify({ id: g.id, name: g.name, cover: g.background_image || '', year: g.released?.split("-")[0] || "" })})'>
    ...
    resultsEl.innerHTML = `<div class="loading-msg" style="color:var(--text-2)">
      Couldn't reach RAWG API. <a href="https://rawg.io/apidocs" target="_blank" style="color:var(--accent)">Get a free key →</a>
      <br><br>
...
window.handleAddGame = (game) => {
  ...
  document.querySelectorAll('.result-add-btn').forEach(btn => {
    if (btn.closest('.search-result-item')?.querySelector('.result-name')?.textContent === game.name) {
      btn.textContent = '✓ In backlog';
    }
  });
};

// Fallback: add a mock game for demo purposes
window.addMockGame = () => {
...
function updateFavoriteTeam(team) {
  ...
  if (statFavorite) {
    statFavorite.textContent = team ? team.split(' ').pop() : '—';
  }
  ...
  // Re-render games with new favorite highlighted
  loadGames();
  loadStandings();
}
